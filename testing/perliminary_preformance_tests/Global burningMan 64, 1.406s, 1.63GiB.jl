...
function fillAdjacent()
    adjacent = zeros(Int64, N, 4)
    for i in 1:N
        adjacent[i, :] = findAdjacent(i)
    end
    return adjacent
end

function findAdjacent(i::Int64)
...
function explore_cluster_at(i::Int64, c::Int64)
    ...
    while nr_unexplored > nr_explored
        ...
        nr_unexplored = check_neighbours(current_fiber, nr_unexplored, c)
    end
end
...
function check_neighbours(current_fiber::Int64, nr_unexplored::Int64, c::Int64)
    ...
    for adj_fiber in adjacent[current_fiber, :]
        ...
        if s == BROKEN
            ...
            nr_unexplored = add_unexplored(adj_fiber, nr_unexplored)
...
function add_unexplored(i::Int64, nr_unexplored::Int64)
    nr_unexplored += 1
...
function show(v=status)
    display(reshape(v,(L,L)))
end
...
# Define constants
const ALIVE::Int64 = -1 # A fiber that has not yet been broken
const CURRENT_BORDER::Int64 = -2 # The border of the current cluster being explored
const PAST_BORDER::Int64 = -3 # The border of a cluster that has been explored
const BROKEN::Int64 = 0 # A broken fiber that has not been explored
# Status larger than 0 represents the cluster id that the fiber belongs to

const L::Int64 = 64
const N::Int64 = L*L # Number of fibers
const x::Vector{Float64} = rand(Float64, N) # Max extension
const σ::Vector{Float64}  = ones(Float64, N) # Relative tension
const adjacent::Array{Int64, 2}  = fillAdjacent()
const status::Vector{Int64} = fill(-1, N)
const cluster_size::Vector{Int64} = zeros(Int64, N)
const cluster_outline::Vector{Int64} = zeros(Int64, N)
const cluster_outline_length::Vector{Int64} = zeros(Int64, N)
const unexplored::Vector{Int64} = zeros(Int64, N)
