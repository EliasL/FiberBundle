...
function findNextFiber(;args...)
    ...
    # by dividing by x.
    return argmax(args[:σ] ./ args[:x])
end

function break_fiber(i;args...)
    status = args[:status]
    σ = args[:σ]
    status[i] = BROKEN
    σ[i] = 0
...
function update_σ(;args...)
    ...
    for i in eachindex(status)
        ...
        if status[i] == BROKEN
            ...
            explore_cluster_at(i, c; args...)
            # We should now have updated cluster_outline,
            # and with that we can update sigma for one cluster
            update_cluster_outline_stress(c; args...)
        end
    end
end

function explore_cluster_at(i::Int64, c::Int64; args...)
    # We explore the cluster of broken fibers and
    # map the border of the cluster

    unexplored = args[:unexplored]
    cluster_size = args[:cluster_size]
    cluster_outline_length = args[:cluster_outline_length]
    ...
    while nr_unexplored > nr_explored
        ...
        nr_unexplored = check_neighbours(current_fiber, nr_unexplored, c; args...)
    end
end

function check_neighbours(current_fiber::Int64, nr_unexplored::Int64, c::Int64; args...)
    ...
    unexplored = args[:unexplored]
    cluster_size = args[:cluster_size]
    cluster_outline = args[:cluster_outline]
    cluster_outline_length = args[:cluster_outline_length]
...
function update_cluster_outline_stress(c::Int64; args...)
...
Args = Dict(
    :L => 0,
    :N => 0, # Number of fibers
    :x => rand(Float64, 0), # Max extension
    :σ  => ones(Float64, 0), # Relative tension
    :adjacent => fillAdjacent(0),
    :status => fill(-1, 0),
    :cluster_size => zeros(Int64, 0),
    :cluster_outline => zeros(Int64, 0),
    :cluster_outline_length => zeros(Int64, 0),
    :unexplored => zeros(Int64, 0)
    )
