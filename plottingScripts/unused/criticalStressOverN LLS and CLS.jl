...
function otherPropertiesPlot(L, ts, NR, dist; use_y_lable=true, add_ELS=true)
    ...
    function make_plot4(X, Y, ylabel; 
        ...
        scatter!(X, permutedims(Y[:, :, 2]), label="", linestyle=:solid,
        c=colors[2], markerstrokecolor=colors[2],
        legend=position, markershape=markershape, markersize=7)       
        scatter!(X, permutedims(Y[:, :, 1]), label="",
        c=colors[1], markerstrokecolor=colors[1], linestyle=:dash,
        legend=position, markershape=markershape, markersize=4)
        plot!(X, 1.25*X.+2.25, label=L"\frac{5}{4}\ln(\ln(N))+2.25", c=:black)
        plot!([], [], label=" ", alpha=0)
...
#ts = (0:7) ./ 10
data_path = "newData/"
ts = reverse!([0.05, 0.1, 0.3, 0.35, 0.4, 0.5])
