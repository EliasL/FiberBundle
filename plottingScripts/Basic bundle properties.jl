...
function basicPropertiesPlot(L, ts, nr)
    ...
    nr_clusters_plot = plot(k_N, nr_clusters, label = lables,
    xlabel=L"k/N", ylabel=L"\#/N", plot_titlevspan=-0.1)#, title="Relative number of clusters")
    add_spanning_point(nr_clusters)

    largest_cluster_plot = plot(k_N, largest_cluster, label = lables,
    xlabel=L"k/N", ylabel=L"S_{\mathrm{max}}/N", plot_titlevspan=-0.1)#, title="Size of largest cluster")
    add_spanning_point(largest_cluster)

    largest_perimiter_plot = plot(k_N, largest_perimiter, label = lables,
    xlabel=L"k/N", ylabel=L"H_{\mathrm{max}}/N")#, title="Length of the longest perimeter",)
    add_spanning_point(largest_perimiter)

    most_stressed_fiber_plot = plot(k_N, most_stressed_fiber, label = lables,
    xlabel=L"k/N", ylabel=L"σ")#, title="Stress of most stressed fiber")    
    add_spanning_point(most_stressed_fiber)

    l = @layout [
        A B; C D
    ]
    p = plot(nr_clusters_plot, largest_cluster_plot, largest_perimiter_plot, most_stressed_fiber_plot, layout=l,
    plot_title="$nr "*L"L="*"$L")
    return p
end
...
nr = "LLS"
α = 2.0
lls = basicPropertiesPlot(L, ts, "LLS")
cls = basicPropertiesPlot(L, ts, "CLS")

l = @layout [
    A; B
]
plot(lls, cls, layout=l, size=(900, 1000), left_margin=2Plots.mm)
