...
function get_fit(x, y)
    upperbounds = [ 20,  20, -100 , -0.30]
    lowerbounds = [-10, -10, -1000, -0.80]
    p0 = [0.1, 0.9, -100, -0.55]
...
function otherPropertiesPlot(L, ts, NR; use_y_lable=true, add_ELS=true)
    ...
    c = -0.1
    d = 0.1
    largest_cluster_spanning = get_data_spanning(L, nr, ts, "average_largest_cluster", ex=a)
    largest_perimeter_spanning = get_data_spanning(L, nr, ts, "average_largest_perimiter", ex=b)
    most_stressed_fiber_spanning = get_data_spanning(L, nr, ts, "average_most_stressed_fiber", ex=c)
...
#=     y = largest_perimeter_spanning[:, :, 1]
    ...
    largest_cluster_spanning = get_data_spanning(L, nr, ts, "average_largest_cluster", ex=0)
    largest_perimeter_spanning = get_data_spanning(L, nr, ts, "average_largest_perimiter", ex=0)
    x1 = most_stressed_fiber_spanning[:, :, 1]
    x2 = most_stressed_fiber_spanning[:, :, 2]
    y1 = largest_cluster_spanning[:, :, 1] ./ largest_perimeter_spanning[:, :, 1] ./ permutedims(log.(log.(log.(L))).^d)
    y2 = largest_cluster_spanning[:, :, 2] ./ largest_perimeter_spanning[:, :, 2] ./ permutedims(L.^d)
...
p = plot(plots..., size=(620,930), layout = @layout([ A B; C D; E F]))
