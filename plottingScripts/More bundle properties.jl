...
function otherPropertiesPlot(L, ts, NR; use_y_lable=true, add_ELS=true)
    ...
    get_spanning_data(key; divide=N) = [[y[round(Int64, x)] for (x,y) in zip(spanning_x[i], data_nr)] for (i,data_nr) in enumerate(get_data(key, divide=divide))]
    ...
    function make_plot(y, ylabel, labels; x=k_N, title="", ylims=(-Inf, Inf), xlabel="", xlims=(-Inf, Inf),
        ...
        xlabel=xlabel, ylabel=yLabel(ylabel), title=title, markershape=:auto, mc=:auto, msc=:auto,
    ...
    function make_plot2(y, ylabel, labels; x=k_N, title="", ylims=(-Inf, Inf), xlabel="", xlims=(-Inf, Inf),
        ...
        p = plot(x, y, label = labels, legend=position, xlims=xlims, ylims=ylims,  markersize=5, 
    ...
    σ_scaled = make_plot2([most_stressed_fiber_nr for most_stressed_fiber_nr in most_stressed_fiber[2][1:length(ts2)]./most_stressed_fiber[2][1:length(ts2)][]],
                        position=:topleft, L"Area under $σ$", permutedims(ts2), x=[k_N for _ in ts], xlabel=L"t_0")
