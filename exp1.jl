...
# ╔═╡ e7a74283-6359-4ec5-8d25-581706d8e441
using BenchmarkTools, Plots, Random, ProgressMeter, PlutoUI, PlotlyJS

# ╔═╡ 44d0b4c6-20c5-4ead-9c7b-677c7ed83c2d
include("burningMan.jl")

# ╔═╡ b11def3c-70d4-4d5d-b517-d933f6fa3c69
plotlyjs()

# ╔═╡ 2655b663-c689-44f2-b6a0-c68dae183d33
function main(L)
	...
	status_storage = zeros(Float64, N, L,L)
	
	tension_storage = zeros(Float64, N, L,L)
    ...
    @showprogress for step in 1:N
		...
		status_storage[step, :, :] = clamp.(reshape(status, (L,L)),0, 10000)
		map!(x -> x==0 ? NaN : x, status_storage, status_storage)
		tension_storage[step, :, :] = reshape(σ./x, (L,L)) 
    end
	return (status_storage, tension_storage)
end

# ╔═╡ b3a49b15-e96d-4479-b895-d18624914b48
function hm(data)
...
# ╔═╡ 4b630b0c-4a94-40f2-8086-d1a5041fcb53
function show_grid(data, index)
	data1 = data[1]
	data2 = data[2]
	hms = [
		hm(data1[index, :, :]),
    ...
    Plots.plot(hms..., axis=([], false))
end

# ╔═╡ 2e1aeb06-029a-47a9-b9df-41c29b8dc551
L=20

# ╔═╡ 0950b4bd-7d21-4ca2-96e7-42315afc6896
begin
	data = main(L)
	print("")
end

# ╔═╡ 247c4304-15ce-4c64-b641-702102967fe0
@bind index Slider(1:L*L, default = 1)

# ╔═╡ a7fd5de3-098b-4af2-ad23-b477c24b2541
show_grid(data, index)
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "c37e77b666c8a739ad28155b4019a6e3cf4b5dea"
